...
def inject_lab_weight_ghost(lab_data=None):
    ...
    print(f"👻 Lab Smart Injection Started (Target Job: {actual_site_job_card}, Lot: {target_lot_num})...")
    try:
        with sync_playwright() as p:
                ...
                if "SamplingweightingDeatils" in page.url:
                    if excel_job_card != "UNKNOWN":
                        try:
                            ...
                            # Match always using the REAL job card number
            ...
            # 🚀 LOT DROPDOWN SELECTOR LOGIC (Naya Hissa)
            if target_lot_num != "1":
                try:
                    print(f"🔄 Switching BIS Website to Lot {target_lot_num}...")
                    # 1. Try finding modern Select2 dropdown
                    lot_dropdown = bis_page.locator("span[class*='select2-selection']").first
                    if lot_dropdown.count() > 0:
                        lot_dropdown.click()
                        bis_page.wait_for_timeout(500)
                        # Match 'Lot 2' text
                        option = bis_page.locator(f"li.select2-results__option:has-text('Lot {target_lot_num}')").first
                        if option.count() > 0:
                            option.click()
                            bis_page.wait_for_timeout(2000) # Wait for data to refresh
                    else:
                        # 2. Try classic select dropdown
                        select_el = bis_page.locator("select").filter(has_text=f"Lot {target_lot_num}").first
                        if select_el.count() > 0:
                            select_el.select_option(label=f"Lot {target_lot_num}")
                            bis_page.wait_for_timeout(2000)
                except Exception as e:
                    print(f"⚠️ Warning: Could not select Lot {target_lot_num}: {e}")
    sample_wt = lab_data.pop("sample_drawn_wt", None)
    button_wt = lab_data.pop("button_wt", None)
    ...
    try:
        with sync_playwright() as p:
                ...
                if "SamplingweightingDeatils" in page.url:
                    if excel_job_card != "UNKNOWN":
                        try:
                            ...
                            # Yahan excel_job_card ki jagah actual_site_job_card aayega
                            if actual_site_job_card in site_job_card:
                                bis_page = page; break 
                        except: continue 
                    else:
            ...
            if not bis_page:
                ...
                # Error message me bhi actual_site_job_card dikhayein
                return f"❌ अलर्ट: Job Card '{actual_site_job_card}' साइट पर मैच नहीं हुआ!"
