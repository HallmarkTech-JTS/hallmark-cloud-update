...
def inject_reception_weight_ghost(job_id, job_data, delay_ms=1500):
    ...
    try:
        with sync_playwright() as p:
            ...
            for page in browser.contexts[0].pages:
                try:
                    # Yahan job_id ki jagah search_job_id aayega
                    if search_job_id in page.locator("body").inner_text(): job_matched = True; break
                except: pass
                for frame in page.frames:
                    try:
                        # Yahan bhi search_job_id aayega
            ...
            if not job_matched:
                ...
                # Error message me bhi search_job_id dikhayein
                        ...
                        if weight_input.count() > 0:
                            ...
                            if current_val != weight:
                                ...
                                if save_btn.is_visible():
                                    ...
                                    # 🚀 SMART INTERNET WAIT (Slow net aur missed tags ka fix)
                                    print(f"⏳ Tag {tag_id} saved. Waiting for BIS portal to stabilize...")
                                    time.sleep(1.5) # Mandatory JS animation wait
                                    try:
                                        # Agar net slow hai, toh table wapas load hone ka 5 sec tak wait karega
                                    ...
                                    time.sleep(delay_ms / 1000.0) # User ka manual delay
    ...
    try:
        with sync_playwright() as p:
            ...
            () => {
                ...
                for (let r of rows) {
                    let cells = r.querySelectorAll('td');
                    // Index 1=Tag, 2=Material, 3=Item Category, 4=HUID
                    if (cells.length >= 5) {
